...
func declare_intent(active_enemies: Array):
	...
	# Special logic for Goblin Warchief
		...
		next_action = _get_goblin_warchief_intent(active_enemies)
	# Special logic for D6 Healer
	elif enemy_data.enemy_name == "D6":
		next_action = _get_d6_intent(active_enemies)
	# Specific enemies with two actions alternate between them.
		...
		next_action = _get_alternating_intent()
	elif enemy_data.enemy_name == "White Knight":
		next_action = _get_white_knight_intent(active_enemies)
	else:
		next_action = _get_default_intent(active_enemies)
...
func _get_evil_dice_tower_intent(active_enemies: Array) -> EnemyAction:
	if _turn_count == 1:
...
func _get_goblin_warchief_intent(active_enemies: Array) -> EnemyAction:
	...
	if not possible_actions.is_empty():
		return possible_actions.pick_random()
	return enemy_data.action_pool[0]

func _get_d6_intent(active_enemies: Array) -> EnemyAction:
...
func _get_alternating_intent() -> EnemyAction:
	# The _is_charging flag is used to track which action to use.
	# false = action_pool[0], true = action_pool[1]
	if _is_charging:
		_is_charging = false
		return enemy_data.action_pool[1]
	else:
		_is_charging = true
		return enemy_data.action_pool[0]

func _get_white_knight_intent(active_enemies: Array) -> EnemyAction:
	...
	if possible_actions.is_empty():
		...
		return enemy_data.action_pool.filter(func(a): return a.action_name == "Incel-sior")[0]
	return possible_actions.pick_random()

func _get_default_intent(active_enemies: Array) -> EnemyAction:
	var possible_actions = enemy_data.action_pool.duplicate()
	var injured_allies = active_enemies.filter(func(e): return e != self and e.hp < e.max_hp)
	...
	return possible_actions.pick_random() if not possible_actions.is_empty() else enemy_data.action_pool.pick_random()
